package mplanweb.music.web.search;

public class MsearchDTO {

	String art_artist;
	String art_img;
	String art_debut;
	String al_artist;
	String al_album;
	String al_year;
	String al_imgorg;

	String m_artist;
	String m_title;
	String art_label;
	String m_titleuse;
	
	
	
	public String getM_titleuse() {
		return m_titleuse;
	}
	public void setM_titleuse(String m_titleuse) {
		this.m_titleuse = m_titleuse;
	}
	String m_album;
	String m_year;
	String m_imgo;
	String m_320ko;
	String m_192ko;
	
	String art_hit;
	String art_etc;
	String al_corp;
	
	String mv_artist;
	String mv_title;
	String mv_album;
	String mv_year;
	String mv_imgo;
	String mv_mvko;
	String mv_age;
	
	String mv_release;
	
	public String getMv_release() {
		return mv_release;
	}
	public void setMv_release(String mv_release) {
		this.mv_release = mv_release;
	}
	public String getMv_artist() {
		return mv_artist;
	}
	public void setMv_artist(String mv_artist) {
		this.mv_artist = mv_artist;
	}
	public String getMv_title() {
		return mv_title;
	}
	public void setMv_title(String mv_title) {
		this.mv_title = mv_title;
	}
	public String getMv_album() {
		return mv_album;
	}
	public void setMv_album(String mv_album) {
		this.mv_album = mv_album;
	}
	public String getMv_year() {
		return mv_year;
	}
	public void setMv_year(String mv_year) {
		this.mv_year = mv_year;
	}
	public String getMv_imgo() {
		return mv_imgo;
	}
	public void setMv_imgo(String mv_imgo) {
		this.mv_imgo = mv_imgo;
	}
	public String getMv_mvko() {
		return mv_mvko;
	}
	public void setMv_mvko(String mv_mvko) {
		this.mv_mvko = mv_mvko;
	}
	public String getMv_age() {
		return mv_age;
	}
	public void setMv_age(String mv_age) {
		this.mv_age = mv_age;
	}
	public String getAl_corp() {
		return al_corp;
	}
	public void setAl_corp(String al_corp) {
		this.al_corp = al_corp;
	}
	public String getArt_label() {
		return art_label;
	}
	public void setArt_label(String art_label) {
		this.art_label = art_label;
	}
	public String getArt_hit() {
		return art_hit;
	}
	public void setArt_hit(String art_hit) {
		this.art_hit = art_hit;
	}
	public String getArt_etc() {
		return art_etc;
	}
	public void setArt_etc(String art_etc) {
		this.art_etc = art_etc;
	}
	public String getArt_artist() {
		return art_artist;
	}
	public void setArt_artist(String art_artist) {
		this.art_artist = art_artist;
	}
	public String getArt_img() {
		return art_img;
	}
	public void setArt_img(String art_img) {
		this.art_img = art_img;
	}
	public String getArt_debut() {
		return art_debut;
	}
	public void setArt_debut(String art_debut) {
		this.art_debut = art_debut;
	}
	public String getAl_artist() {
		return al_artist;
	}
	public void setAl_artist(String al_artist) {
		this.al_artist = al_artist;
	}
	public String getAl_album() {
		return al_album;
	}
	public void setAl_album(String al_album) {
		this.al_album = al_album;
	}
	public String getAl_year() {
		return al_year;
	}
	public void setAl_year(String al_year) {
		this.al_year = al_year;
	}
	public String getAl_imgorg() {
		return al_imgorg;
	}
	public void setAl_imgorg(String al_imgorg) {
		this.al_imgorg = al_imgorg;
	}
	public String getM_artist() {
		return m_artist;
	}
	public void setM_artist(String m_artist) {
		this.m_artist = m_artist;
	}
	public String getM_title() {
		return m_title;
	}
	public void setM_title(String m_title) {
		this.m_title = m_title;
	}
	public String getM_album() {
		return m_album;
	}
	public void setM_album(String m_album) {
		this.m_album = m_album;
	}
	public String getM_year() {
		return m_year;
	}
	public void setM_year(String m_year) {
		this.m_year = m_year;
	}
	public String getM_imgo() {
		return m_imgo;
	}
	public void setM_imgo(String m_imgo) {
		this.m_imgo = m_imgo;
	}
	public String getM_320ko() {
		return m_320ko;
	}
	public void setM_320ko(String m_320ko) {
		this.m_320ko = m_320ko;
	}
	public String getM_192ko() {
		return m_192ko;
	}
	public void setM_192ko(String m_192ko) {
		this.m_192ko = m_192ko;
	}


}
